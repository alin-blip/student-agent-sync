...
BEGIN
  IF NEW.status IN ('withdrawn', 'rejected', 'cancelled', 'canceled') THEN
...
UPDATE public.commission_snapshots cs
SET snapshot_status = 'cancelled'
FROM public.enrollments e
WHERE cs.enrollment_id = e.id
  AND e.status IN ('withdrawn', 'rejected', 'cancelled', 'canceled')
  AND cs.snapshot_status <> 'cancelled';