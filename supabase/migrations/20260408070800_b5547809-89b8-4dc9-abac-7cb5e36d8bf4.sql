...
BEGIN
  -- ═══════════════════════════════════════════════════════════
  -- REVERSAL: When enrollment moves to a terminal negative status,
  -- cancel the associated commission snapshot
  -- ═══════════════════════════════════════════════════════════
  IF NEW.status IN ('withdrawn', 'rejected', 'cancelled')
     AND (OLD.status IS NULL OR OLD.status NOT IN ('withdrawn', 'rejected', 'cancelled')) THEN

    UPDATE commission_snapshots
    SET snapshot_status = 'cancelled'
    WHERE enrollment_id = NEW.id
      AND snapshot_status NOT IN ('cancelled');
