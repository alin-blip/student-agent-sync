...
-- 1. Fix enrollments_status_check to include paid_by_university
...
ALTER TABLE public.enrollments ADD CONSTRAINT enrollments_status_check
  CHECK (status IN ('applied', 'documents_pending', 'documents_submitted', 'processing', 'offer_received', 'accepted', 'funding', 'enrolled', 'active', 'rejected', 'withdrawn', 'paid_by_university'));

-- 2. Replace create_commission_snapshot() to create snapshots from 'funding' onwards
...
BEGIN
  ...
  -- When funding_status becomes approved, mark eligibility (snapshot already exists)
  IF NEW.funding_status = 'approved' AND (OLD.funding_status IS DISTINCT FROM 'approved') THEN
    -- If no snapshot yet (edge case), create one now using same logic
    SELECT id INTO _existing FROM commission_snapshots WHERE enrollment_id = NEW.id;
    IF _existing IS NULL THEN
      ...
      IF _agent_rate = 0 THEN
        SELECT count(*) INTO _count FROM enrollments e JOIN students s ON e.student_id = s.id
          WHERE s.agent_id = _agent_id AND e.status IN ('funding', 'enrolled', 'active', 'paid_by_university');
        SELECT ct.commission_per_student, ct.tier_name INTO _tier_rate, _tier_name FROM commission_tiers ct
          WHERE ct.university_id IS NULL AND _count >= ct.min_students AND (ct.max_students IS NULL OR _count <= ct.max_students)
          ORDER BY ct.min_students DESC LIMIT 1;
      ...
      VALUES (NEW.id, _agent_id, _admin_id, _uni_id, COALESCE(_agent_rate, 0), COALESCE(_admin_rate, 0), COALESCE(_rate_source, 'None'), 'pending_25', now());
    END IF;
    -- No status change needed - already pending_25
  END IF;
